...
- `TodoWrite` → Track tasks with a Markdown todo list in your response
- `Task` tool with subagents → Say subagents aren't available in Gemini CLI and do the work directly
- `Skill` tool → `~/.gemini/superpowers/.gemini/superpowers-gemini use-skill` command (already available)
- `Read`, `Write`, `Edit`, `Bash` → Use Gemini CLI built-in tools (file system + shell)
- Web fetching/search → Use Gemini CLI web tools when needed
...
- Skills with checklists require a todo list with each item
